...
async def analyze_company(name: str = Query(..., description="Company name")):
    ...
    linkedin, glassdoor, crunchbase, news = await asyncio.gather(
        linkedin_task, glassdoor_task, crunchbase_task, news_task
    )
    ...
    return CompanyResponse(summary=summary_result.summary, rating=summary_result.rating)
